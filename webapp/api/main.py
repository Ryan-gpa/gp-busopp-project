...
def backfill_apollo_metrics(limit: int = 10):
    ...
    debug_data["companies_cache"] = [{"id": r[0], "rev": json.loads(r[1]).get("annual_revenue") if r[1] else None} for r in rows]
    
    contact_rows = conn.execute("SELECT org_id FROM contacts_cache").fetchall()
    debug_data["contacts_cache"] = [r[0] for r in contact_rows]
    
    conn.close()
    
    return {"message": "Debug info", "debug": debug_data}
